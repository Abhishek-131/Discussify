...
const CommunityResources: React.FC = () => {
  ...
  const fetchResources = async () => {
    try {
      const res = await API.get(`/resources?community=${id}`);
      setResources(res.data.resources || []);
    } catch (err: any) {
      setMessage(err.response?.data?.error || "Failed to load resources");
    }
  };
  ...
  const handleShare = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFile(null);
      setShowModal(false);
      setMessage("✅ Resource shared to community!");
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h3 className="mb-0">Community Resources</h3>
        <button
              ...
              <div className="modal-content">
                <form onSubmit={handleShare}>
                  <div className="modal-header">
                    <h5 className="modal-title">Share Resource in Community</h5>
                    <button
                      type="button"
                      className="btn-close"
      ...
      <div className="row">
        {resources.map((r) => (
          <div className="col-md-6 mb-4" key={r._id}>
            <div className="card feed-card h-100">
              ...
              {/* ✅ Always show external link if present */}
              {r.externalLink && (
        ...
        {resources.length === 0 && (
          <p className="text-center text-muted">
            No resources yet in this community. Be the first to share!
          </p>
        )}
      </div>
    </div>
